document.addEventListener('DOMContentLoaded', function() {
    // 手机号验证相关元素
    const phoneVerificationOverlay = document.getElementById('phone-verification-overlay');
    const phoneNumberInput = document.getElementById('phone-number');
    const phoneErrorMessage = document.getElementById('phone-error');
    const verifyBtn = document.getElementById('verify-btn');
    
    // 注释或删除以下代码块，使验证框始终显示
    // const isVerified = localStorage.getItem('phoneVerified');
    // if (isVerified) {
    //     phoneVerificationOverlay.style.display = 'none';
    // }
    ...
    function verifyPhoneNumber() {
        ...
        if (!phoneNumber) {
            phoneErrorMessage.textContent = '请输入手机号码';
            return;
        }
        
        if (!phoneRegex.test(phoneNumber)) {
            phoneErrorMessage.textContent = '请输入正确的手机号码';
            return;
        }
    ...
    const reduceTermRadio = document.getElementById('reduce-term');
    const reducePaymentRadio = document.getElementById('reduce-payment');
    const partialAmountDisplay = document.getElementById('partial-amount-display');
    const savedInterestReduceTermElement = document.getElementById('saved-interest-reduce-term');
    const newTermElement = document.getElementById('new-term');
    ...
    function analyzeEarlyRepayment() {
        ...
        // 部分提前还款分析
        analyzePartialEarlyRepayment(currentPeriod);
    }
    
    // 新增部分提前还款分析函数
    function analyzePartialEarlyRepayment(currentPeriod) {
        // 获取部分提前还款金额（万元转换为元）
        const partialAmount = parseFloat(partialAmountInput.value) * 10000;
        if (isNaN(partialAmount) || partialAmount <= 0) {
            alert('请输入有效的部分提前还款金额！');
            return;
        }
        
        // 获取当前贷款信息
        const currentPayment = globalPaymentSchedule[currentPeriod - 1];
        const remainingPrincipal = currentPayment.remainingPrincipal;
        
        // 检查部分提前还款金额是否超过剩余本金
        if (partialAmount >= remainingPrincipal) {
            alert('部分提前还款金额不能大于或等于剩余本金！');
            return;
        }
        
        // 更新部分提前还款金额显示
        partialAmountDisplay.textContent = formatCurrency(partialAmount);
        ...
        // 计算剩余期数
        const remainingMonths = totalMonths - currentPeriod;
        ...
        if (isEqualPrincipalInterest) {
            // 等额本息
            const monthlyPayment = globalPaymentSchedule[0].payment;
            
            // 计算新的贷款期限
            newTerm = Math.ceil(Math.log(monthlyPayment / (monthlyPayment - newRemainingPrincipal * monthlyInterestRate)) / 
            ...
            for (let i = currentPeriod; i < globalPaymentSchedule.length; i++) {
                originalRemainingInterest += globalPaymentSchedule[i].interest;
            }
        ...
        if (isEqualPrincipalInterest) {
            // 等额本息
            // 计算新的月供
            newMonthlyPayment = newRemainingPrincipal * monthlyInterestRate * Math.pow(1 + monthlyInterestRate, remainingMonths) / 
            ...
            // 等额本金
            // 计算新的月供（首月）
            const newMonthlyPrincipal = newRemainingPrincipal / remainingMonths;
        ...
        newTermElement.textContent = newTerm + '期';
    ...
    function showComparisonModal(period) {
        ...
        // 获取当前期数的还款信息
        const currentPayment = globalPaymentSchedule[period - 1];
        ...
        if (earlyRepaymentOption.checked) {
            ...
            // 分析部分提前还款
            analyzePartialEarlyRepayment(period);
        }
    }
    ...
    function updatePaymentScheduleTable(paymentSchedule) {
        ...
        paymentSchedule.forEach(payment => {
            ...
            row.innerHTML = `
                ...
                <td><button class="analyze-period-btn">分析</button></td>
            `;
            
            // 添加点击事件到分析按钮
            const analyzeBtn = row.querySelector('.analyze-period-btn');
